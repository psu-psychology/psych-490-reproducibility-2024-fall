...
We discussed this study on [2024-10-11](../notes/wk07-2024-10-11-qrps.qmd).
...
:::  {.callout-note}
My graduate statistics professor, [Dr. Mike Meyer](https://www.linkedin.com/in/mike-meyer-0056578a/), always exhorted us to "plot our data!"
...
This is how we tell the `aov()` command that the linear combination (sum) of `song` and `father_age` are the variables we want to use to predict `old_feeling`.
...
That is, what if `father_age` was systematically related to the order in which we sampled participants.
